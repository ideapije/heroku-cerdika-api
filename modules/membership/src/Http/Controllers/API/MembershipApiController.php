...
class MembershipApiController extends Controller
{
    ...
    public function login(LoginUser $request, Account $user)
    {
    ...
    public function store(StoreUser $request, Account $user)
    {
        DB::beginTransaction();
        try {
    ...
    public function resendVerify(ResetRequest $request, Account $user)
    {
        $user = $user->where('email', $request->email)->first();
        if ($user) {
